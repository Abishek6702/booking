...
import { Mail, Lock, Eye, EyeOff, Building2, AlertCircle, Compass, ShieldCheck, Sparkles } from "lucide-react";
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (serverUser.role !== "OWNER") {
        throw new Error("Access denied. This portal is for resort owners only.");
      }

      login(serverUser, tokens);

      // Check owner approval before granting dashboard access. 
      // Admins should not bypass this if they are entering the owner-specific portal.
      const isApproved = serverUser.ownerStatus === "APPROVED";
      if (!isApproved) {
        toast.info("Your account is pending owner verification.");
        ...
        <div className="bg-transparent p-0">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none mb-2">
              Property <span className="text-primary italic font-light font-serif">Login</span>
            </h1>
            <p className="text-sm text-slate-500 font-medium">Manage your premium properties.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <Input
                  ...
                  type="email"
                  className={`h-12 pl-11 rounded-2xl bg-white border-slate-200 focus:ring-primary/20 focus:border-primary transition-all text-sm font-medium ${errors.email ? "border-red-400 bg-red-50/50" : ""}`}
                  {...register("email")}
                />
              </div>
              {errors.email && (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between ml-1">
                ...
                <Link to="/owner/forgot-password" size="sm" className="text-[10px] font-black text-primary uppercase tracking-widest hover:underline">
                  Forgot?
                </Link>
              </div>
              <div className="relative group">
          ...
          <div className="mt-10 pt-8 border-t border-slate-100 space-y-6">
            <p className="text-center text-[11px] font-bold text-slate-400 uppercase tracking-widest">
              New owner?{" "}
              <Link to="/owner/signup" className="text-primary hover:underline ml-1">Register Here</Link>
            </p>
